...
|---|---|---|---|---|---|---|
| 3.1 | Cumulant D3Q19 viscosity-offset correction: `omega_eff = omega_shear·(1 + 0.0025 − 0.16·u²)`, `.min(2.0)`, D3Q19-only branch | kernels.rs:415-416 | **B-conditional** | PHYSICS.md documents it (Geier et al. refs, TGV3D order 1.91, Couette/Poiseuille null gates) BUT the 0.0025 was fitted to the TGV3D decay — a constant calibrated against the acceptance observable, and the O(u²) defect coefficient 0.16 should be derivable analytically for the lattice rather than fitted | (a) inline comment citing PHYSICS.md + refs; (b) **adversarial audit row queued (accuracy-audit Order E)**: ν_eff(u, N) functional-form sweep — the correction must cancel the defect across u AND resolutions, not only at the calibration point; if the offset is resolution-dependent this is a (C) reclassification | P1 (verify), P2 (comment) |
