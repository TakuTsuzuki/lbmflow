...
| Central-moment (a.k.a. `cumulant`) collision | Naming: the `cumulant` option is a cascaded **central-moment** operator (second-order deviatoric shear relaxed at `omega_shear`, higher moments at 1.0), not a logarithmic-cumulant LBM. The D3Q19 path carries a `-0.16 \|u\|^2` velocity-dependent shear-rate correction. Standing verdict (2026-07-07): the calibrated term is **retained with a narrowed claim** — removing it was tested and made the advected-TGV3D frame spread 2.5× worse (removal hypothesis falsified), so it compensates a real \|u\|²-scaling part of the D3Q19 defect; but the residual anisotropic part is uncorrected and **Galilean invariance at finite frame velocity is NOT established on D3Q19** (holdout stays `#[ignore = FINDING]` at its derived band). Use TRT/f64 or D3Q27 references for validation-grade frame-shift studies. | `crates/lbm-core/src/kernels.rs` (`-0.16 * usq`), `crates/lbm-core/src/params.rs:11-16`, `crates/lbm-core/tests/cumulant_holdout.rs`, `docs/PHYSICS.md` (ANOM-P4-008 + Galilean falsification record), `docs/proposals/CUMULANT_GALILEAN_FIX.md` |
