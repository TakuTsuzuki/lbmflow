...
| Central-moment (a.k.a. `cumulant`) collision | Naming: the `cumulant` option is a cascaded **central-moment** operator (second-order deviatoric shear relaxed at `omega_shear`, higher moments at 1.0), not a logarithmic-cumulant LBM. It is **characterized, not fully validated**: the D3Q19 path carries a `-0.16 \|u\|^2` velocity-dependent shear-rate correction whose viscosity footprint is ablation-verified as a real Galilean-class effect, but whose coefficient `0.16` is empirically calibrated and fails the Galilean-invariance holdout (`cumulant_holdout.rs` frame-spread). Tracked derive-or-remove open item — use TRT/f64 or D3Q27 references for validation-grade frame-shift studies. | `crates/lbm-core/src/kernels.rs` (`-0.16 * usq`), `crates/lbm-core/src/params.rs:11-16`, `crates/lbm-core/tests/cumulant_holdout.rs`, `docs/PHYSICS.md` (ANOM-P4-008 + E1 ablation A/B record) |
