...
- feq の 0,1,2 次モーメント恒等式（単体テスト）: Σfeq=ρ, Σfeq c=ρu, Σfeq cc = ρ(cs²I+uu)
  （|u|≤0.1 の数点で 1e-14）。
- 角度（f32）: 質量ドリフト ≤ 1e-4（10³step）、力による運動量成長の相対誤差
  ≤ 5e-3（10²step）。一様場では全セルが同一演算をするため丸めバイアスが
  コヒーレントに蓄積し（~1ulp/step、実測 1.3e-3/10²step）、f64 の水準は原理的に
  出ない（PHYSICS.md 参照。Phase 3 の偏差格納方式で改善予定）。
...
- **2D-1（Re=20, 定常）** 参照値: Cd = 5.5795, Cl = 0.0106, Δp* = Δp/(ρU_mean²) = 2.9375
  - D=20（格子 440×82、u_max=0.075, ν=0.1）デフォルトスイート:
    Cd ∈ [5.2, 6.0]、Cl ∈ [−0.05, 0.08]（staircase 粗格子帯）
