...
Source: [REQ_STIRRED_REACTOR.md](REQ_STIRRED_REACTOR.md) §8 (**rev.4**).
...
Source: [DISPERSED_DEPOSITION.md](DISPERSED_DEPOSITION.md) (frozen spec; the
public API surface for T18.1–.3 is frozen in its §5). Tests are adversarially
authored from that spec by orders SEPARATE from the implementations. All
vocabulary stays domain-neutral (particle-laden multiphase flow). Bands marked
"frozen at first measurement" follow the usual protocol: tighten freely,
loosening requires PHYSICS.md rationale.

- **T18.1 Localized interior volume source/sink (CR-1)** —
  `crates/lbm-core/tests/t18_1_interior_source_sink.rs`:
...
- **T18.3 Dispersed-phase deposition layer (CR-3)** —
  `crates/lbm-core/tests/t18_3_particle_deposition.rs`:
  - Terminal velocity: single particle in still fluid under gravity matches the
    analytic Schiller-Naumann settling velocity (reducing to Stokes
    v_s = (ρ_p/ρ_f − 1) g d²/(18ν) at Re_p < 0.1) across a St sweep spanning
    ≥ 2 decades; band ±2% target, frozen at first measurement.
  - Deposition-map determinism: bit-identical deposit records (positions and
    order) when the sampled fluid field comes from an unpartitioned vs a
    T13-partitioned run of the same scenario.
  - Floor-crossing capture: a straight-line trajectory crossing the floor plane
    is recorded at the exact interpolated crossing point; the particle leaves
    the suspended set (n_deposited + n_suspended is conserved).
- **T18.4 Forward-model monotone trend anchors** — example-level, `#[ignore]`
  (runs `--example dispersed_seeding`): (1) ejection rate ↑ → CV ↑;
  (2) agitation present → CV ↑ vs quiescent (quantitative only after P3 —
  stub until then); (3) fill volume ↑ → CV ↑; (4) repeated ejection passes ↑ →
  CV ↓. Regression: gentle CV ∈ [1.05, 1.30], empty_bin_fraction ≤ 0.15,
  Ma ≤ 0.3 / τ ≥ 0.51 guards abort as specified.
- **T18.5 Inverse recovery (P4)** — **stub, not yet implementable**: the
  inverse solver recovers a known-good recipe on a synthetic n\* generated by
  the forward model itself (parameter recovery within tolerance, objective
  value ≤ frozen band).
