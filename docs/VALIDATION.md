...
1. **z 不変 2D-TGV の退化一致**: 3D 格子（N×N×4、z 周期）に z 不変の 2D TGV を
   初期化し、D2Q9 の同一シナリオと**場単位で一致**（f64 ≤1e-12）。
...
2. **矩形ダクト Poiseuille（厳密級数解）**: 断面 a×b の直方体ダクト、体積力駆動、
   ...
   TRT で L∞rel ≤ 1e-3（級数は n≤99 で打切り、収束確認込み）。
   流量 Q の解析値一致 ±0.5%。
3. **球の抗力**: 一様流中の球（staircase）、Re ∈ {20, 100}。
   Cd を momentum-exchange で測定し **Schiller-Naumann 相関**
   Cd = (24/Re)(1 + 0.15 Re^0.687) の ±10%（Re=20: ≈2.09、Re=100: ≈1.09）。
   ブロッケージ ≤ 3%（領域 ≥ 8D）、D ≥ 24 格子。
4. **3D-TGV（真の 3D、低 Re 収束次数）**: u=(sin x cos y cos z, −cos x sin y cos z, 0)
   の古典 TGV を短時間（渦伸縮が弱い t ≤ 0.1/(νk²)）だけ走らせ、初期減衰率が
   拡散極限に一致・N=32→64 で次数 ≥1.7（拡散スケーリング u0∝1/N）。
5. **3D キャビティ（Ku et al. 1987 等）**: 参照表の採取と凍結は M-C 実施時に行う
   （**出典必須・Ghia 誤植事件の教訓**により、隣接点との滑らかさチェックを
   データ取込み時に必ず実施）。
