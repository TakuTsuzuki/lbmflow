...
3. **球の抗力**: 一様流中の球（staircase）、Re ∈ {20, 100}。
   ...
   重いので #[ignore]。デフォルトスイートには D=12・±25% の軽量版、実測 +14.2%）。
   Cd は 500 step 窓の平均（流入↔流出間の弱減衰音響リップル対策、TESTING_NOTES 参照）。
   実測: Re=100 で Cd = 1.1698（+7.2%、合格）。**Re=20 (D=24) は Cd = 2.9551（+13.2%）
   で公称 D 基準の帯を超過 — staircase の流体力学的半径 r+0.5 で再正規化すると
   +0.6%/+7.1%/+2.3% に潰れる（エンジンは正常）。基準の再定義は PM triage 待ち、
   TESTING_NOTES.md 2026-07-05 #2 参照**。
