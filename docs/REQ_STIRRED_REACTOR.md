...
**文書ID**: REQ-M-F-STR / **改訂**: rev.1（codex 敵対的レビュー REQ_STIRRED_REACTOR.review.md の全48件を反映）
...
## 1. 実行時モード構成マトリクス（相互排他）

全モードを実装するが、1 計算あたり各軸から排他的に1つを選択。コンフィグ検証層が不整合な組合せを棄却する。

| 軸 | モード | 排他制約・備考 |
|---|---|---|
| 回転 | `IBM-inertial`（非定常, 既定） / `MRF-frozen-rotor`（定常近似） / `sliding-overset`（基準級） | MRF は IBM 移動翼と併用不可（#6）。位相平均統計は IBM/overset のみ（#37）。 |
| 界面 | `resolved-phasefield`（保存型 Allen-Cahn） / `point-bubble`（Euler-Lagrange） / `hybrid` | 切替判定は `d_b/Δx, d_b/W, Eo, Re_b, α_g, We_b` の複合条件（#12）。hybrid は相間 質量・運動量・スカラー保存則を定義（§5）。 |
| スカラー | `passive` / `active`（フィードバック対象を列挙） | active は density/viscosity/σ/温度のどれに帰還するか明示（#13）。 |
| 粒子結合 | `one-way` / `two-way` / `four-way` | `α_p` または mass-loading 閾値で選択（#16）。反力散布カーネルと運動量保存検証を伴う。 |
| 精度 | `f32-bulk` ＋ `f64-reduction` | 保存量・トルク・界面曲率・縮約は f64 必須（#32, §7）。 |
| 格子 | `uniform`（既定） / `block-AMR`（上位） | AMR は coarse-fine 保存補間・時間ステップ比・検証を満たす場合のみ（#29）。 |
...
- 界面既定を `resolved-phasefield` と `point-bubble` のどちらにするか（解像コスト vs 物質移動忠実度）。
- スカラーを passive 限定にするか active（物性帰還）まで含めるか。
- 初期版で AMR を含めるか一様格子に限定するか（#29 の実装コスト）。
- 精度既定（f32-bulk+f64-reduction で足りるか、界面近傍を全 f64 にするか）。
