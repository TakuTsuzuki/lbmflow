...
|---|---|---|---|
| 3D GPU acceleration (D3Q19) | ME-1 (B-1 → WGSL 3D + C-13 explicit path) | T14-3D ≤1e-5 + ≥1,500 MLUPS single GPU | RED — top priority |
| explicit backend:"gpu" runs on GPU | C-13 (bundled into ME-1) | scenario gpu request honored end-to-end, guard flips from error to run | RED |
| FP16 storage, ×2 grid capacity | ME-2 (C-12) | T16 bands frozen + ≥1.5× MLUPS @2048² | RED |
