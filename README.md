...
| Collision | `CollisionKind` exposes BGK, TRT, and cumulant / central-moment collision. | `CollisionSpec` exposes `bgk`, `trt`, and `cumulant` (cumulant honored on the 3D CPU path only; other paths reject it explicitly). | Landed 2026-07-07; the run manifest records the collision actually used. |
