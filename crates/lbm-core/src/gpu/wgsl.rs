...
pub(crate) const FLAG_GRAVITY: u32 = 65_536;
...
pub(crate) fn generate_with_storage<L: Lattice>(storage: Storage) -> String {
    ...
    s += "            gz = P.gz;\n";
    s += "        }\n";
    s += "        let force_on = f0x != 0.0f || f0y != 0.0f || f0z != 0.0f || gx != 0.0f || gy != 0.0f || gz != 0.0f;\n";
    s += "        var r = 0.0f;\n";
    s += "        var un = 0.0f;\n";
    s += "        var ut1 = 0.0f;\n";
    s += "        var ut2 = 0.0f;\n";
    ...
    s += "            if (force_on) {\n";
    s += "                let f0n = f0x * B.nxr + f0y * B.nyr + f0z * B.nzr;\n";
    s += "                let f0t1 = f0x * B.t1x + f0y * B.t1y + f0z * B.t1z;\n";
    s += "                let f0t2 = f0x * B.t2x + f0y * B.t2y + f0z * B.t2z;\n";
    s += "                let gn = gx * B.nxr + gy * B.nyr + gz * B.nzr;\n";
    s += "                let gt1 = gx * B.t1x + gy * B.t1y + gz * B.t1z;\n";
    s += "                let gt2 = gx * B.t2x + gy * B.t2y + gz * B.t2z;\n";
    s += "                r = (closure - 0.5f * f0n) / (1.0f - un + 0.5f * gn);\n";
    s += "                let inv_r = 1.0f / r;\n";
    s += "                un = un - 0.5f * (f0n * inv_r + gn);\n";
    s += "                ut1 = ut1 - 0.5f * (f0t1 * inv_r + gt1);\n";
    s += "                ut2 = ut2 - 0.5f * (f0t2 * inv_r + gt2);\n";
    s += "            } else {\n";
    s += "                r = closure / (1.0f - un);\n";
    s += "            }\n";
    s += "        } else {\n";
    ...
    s += "                let gt1 = gx * B.t1x + gy * B.t1y + gz * B.t1z;\n";
    s += "                let gt2 = gx * B.t2x + gy * B.t2y + gz * B.t2z;\n";
    s += "                ut1 = -0.5f * (f0t1 * inv_r + gt1);\n";
    s += "                ut2 = -0.5f * (f0t2 * inv_r + gt2);\n";
    s += "            }\n";
    s += "        }\n";
    s += "        if (B.unk_count == 3u) {\n";
