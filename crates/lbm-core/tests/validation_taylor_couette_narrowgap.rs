//! VAL-TAYC-NG / V&V Axis 9.8 rev 4: narrow-gap Taylor-Couette onset.
//!
//! Rev 3 used `R_i/R_o = 12/28 = 0.43`, a wide-gap geometry, while the
//! Rayleigh critical Taylor number used there is a narrow-gap anchor. This
//! revision keeps the validated rotating-IBM inner cylinder from rev 3 but
//! changes the geometry to `R_i = 20`, `R_o = 24`, so `R_i/R_o = 0.833`.
//!
//! Physics anchor: the single-side-rotating, stationary-outer-cylinder scan
//! uses
//!
//! `Ta = Omega_i^2 R_i d^3 / nu^2`, `d = R_o - R_i`.
//!
//! Rayleigh's narrow-gap eigenvalue gives the critical convection analog
//! `Ta = 1708`. For this finite-radius single-inner-rotation definition, the
//! usual narrow-gap conversion is
//! `Ta_c = 1708 * (R_o/R_i)^4 * (1 - eta) / (1 + eta)`, `eta = R_i/R_o`.
//! With `eta = 20/24`, this gives `Ta_c ~= 322`, keeping the whole 3x scan
//! inside the low-Mach range on this lattice. The seed wavelength uses the
//! narrow-gap critical estimate `lambda_c ~= 2d = 8`; with `L_z = 32`, that is
//! axial Fourier mode `m = 4`.
...
const NY: usize = 64;
const NZ: usize = 32;
const CENTER_X: f64 = 32.0;
const CENTER_Y: f64 = 32.0;
const R_I: f64 = 20.0;
const R_O: f64 = 24.0;
const R_O_EFF: f64 = R_O + 1.0;
const R_MID: f64 = 22.0;
const NU: f64 = 1.0 / 60.0;
const ETA: f64 = R_I / R_O;
const TA_CRIT: f64 =
    1708.0 * (R_O * R_O * R_O * R_O) / (R_I * R_I * R_I * R_I) * (1.0 - ETA) / (1.0 + ETA);
const AXIAL_SEED_MODE: usize = 4;
...
const MARKERS_PER_PLANE: usize = 160;
...
fn assert_finite_or_stop(label: &str, step: usize, fields: &VelocityFields, traj: &[(usize, f64)]) {
    ...
    if !finite || !max_u.is_finite() || max_u > DIVERGED_SPEED {
        panic!(
            "STOP-RULE: VAL-TAYC-NG {label} rotating-IBM narrow-gap run diverged at step {step}; \
...
fn axial_seed_delta_ur(x: usize, y: usize, z: usize) -> f64 {
    let (_, _, r) = radius_xy(x, y);
    if !(R_I + 0.75..R_O - 0.75).contains(&r) {
        return 0.0;
    }
...
fn print_spectrum(label: &str, spectrum: &Spectrum) {
    let ratios: Vec<f64> = spectrum
        ...
        .collect();
    println!(
        "VAL-TAYC-NG {label}: Ta={:.6e}, Omega={:.6e}, samples={}, axisymmetric_energy={:.6e}, \
...
#[test]
fn val_tayc_narrowgap_laminar_profile_light() {
    let ta = 0.5 * TA_CRIT;
    let omega = omega_for_ta(ta);
    let body = extruded_inner_cylinder(omega);
    let mut sim = build_solver();
    ...
    println!(
        "VAL-TAYC-NG laminar: Ta={ta:.6e}, Omega={omega:.6e}, samples={count}, \
    ...
    assert!(
        l2_rel <= 0.15,
        "VAL-TAYC-NG laminar circular-Couette L2_rel={l2_rel:.6e} exceeds 15% band"
    );
    assert!(
        linf_rel_ui <= 0.35,
        "VAL-TAYC-NG laminar circular-Couette Linf/U_i={linf_rel_ui:.6e} exceeds behavior band 35%"
    );
}

#[test]
#[ignore = "heavy VAL-TAYC-NG Axis 9.8 / ANOM-P4-024 rev 4: 64x64x32 D3Q19 x three Taylor numbers"]
fn val_tayc_narrowgap_wavy_vortex_onset_heavy() {
    ...
    println!(
        "VAL-TAYC-NG damping/growth: seed_mode={AXIAL_SEED_MODE}, eps={AXIAL_SEED_EPS:.6e}, \
    ...
    assert!(
        ...
        "VAL-TAYC-NG subcritical Ta=0.5Ta_c did not damp the axial seed: start={:.6e}, end={:.6e}, trajectory={:?}",
        sub_start.nonzero_ratio,
        sub_end.nonzero_ratio,
    ...
    assert!(
        ...
        "VAL-TAYC-NG residual finding: Ta=1.5Ta_c narrow-gap rotating IBM still filters/damps the wavy-vortex seed. \
    ...
    assert!(
        ...
        "VAL-TAYC-NG Ta=3Ta_c did not move more energy into higher modes than Ta=1.5Ta_c: \
