...
fn main() {
    ...
    let mut sim: Simulation<f64> = SimConfig {
        ...
        nu,
        edges: Edges {
            left: EdgeBC::PressureOutlet { rho: 1.0 + drho / 2.0 },
            right: EdgeBC::PressureOutlet { rho: 1.0 - drho / 2.0 },
    ...
    println!("u_center(sim) = {umax_sim:.6e}  u_max(theory) = {umax_theory:.6e}  ratio = {:.4}", umax_sim / umax_theory);
    // flow direction check at quarter points
    println!("ux at x=8: {:.3e}, x=32: {:.3e}, x=56: {:.3e}", sim.ux(8, mid), sim.ux(32, mid), sim.ux(56, mid));
